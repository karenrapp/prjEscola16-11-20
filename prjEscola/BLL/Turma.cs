    ...
    public class Turma
    {
        ...
        public void Inserir()
        {
            string meuSQL = " INSERT INTO TB_TURMA(ID_INSTRUTOR, ID_CURSO, DATA_INICIO, DATA_TERMINO, CARGA_HORARIA) VALUES " +
                            "(" + _ID_INSTRUTOR + ", " + _ID_CURSO + ", '" + (_DATA_INICIO) + "', '" + (_DATA_TERMINO) + "', " + _CARGA_HORARIA + ")";
        ...
        {
            string meuSQL = "DELETE FROM TB_TURMA WHERE ID_TURMA = " + CODIGO;
            SqlHelper.ExecuteNonQuery(connString, CommandType.Text, meuSQL);
        ...
        {
            string meuSQL = "SELECT dbo.TB_TURMA.ID_TURMA, dbo.TB_TURMA.ID_INSTRUTOR, dbo.TB_TURMA.ID_CURSO, dbo.TB_TURMA.DATA_INICIO, dbo.TB_TURMA.DATA_TERMINO, "
                + " dbo.TB_TURMA.CARGA_HORARIA, dbo.TB_INSTRUTOR.NOME_INSTRUTOR, dbo.TB_CURSO.DSC_CURSO" 
                + " FROM dbo.TB_TURMA INNER JOIN dbo.TB_INSTRUTOR ON dbo.TB_TURMA.ID_INSTRUTOR = dbo.TB_INSTRUTOR.ID_INSTRUTOR INNER JOIN" 
        ...
        public void MostrarDados_Turmas(Int32 CODIGO)
        {
